...
public class DataBaseHelper extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase database) {
		String CREATE_CONQUERED_TABLE = "CREATE TABLE `conquered` ( `id` INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, `score` INTEGER DEFAULT '0', `date` INTEGER DEFAULT '0', `longitude` REAL DEFAULT '0', `latitude` REAL DEFAULT '0'); CREATE INDEX `long_index` ON `conquered` (`longitude` ASC);CREATE INDEX `lat_index` ON `conquered` (`latitude` ASC);CREATE INDEX `date_index` ON `conquered` (`date` DESC);";
		...
		String CREATE_TRAINING_TABLE = "CREATE TABLE `training` ( `id` INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, `score` INTEGER, `gpx` TEXT, training_type TEXT, `time` INTEGER, `time_active` INTEGER, `moves` INTEGER, `speed_max` REAL, `speed_avg` REAL, `tempo_min` REAL, `tempo_avg` REAL, `distance` REAL, `altitude_min` INTEGER, `altitude_max` INTEGER, `altitude_upward` INTEGER, `altitude_downward` INTEGER);";
